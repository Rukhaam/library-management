...
import { Library, KeyRound, ArrowLeft } from "lucide-react"; 
...
const ForgotPassword = () => {
  ...
  useEffect(() => {
    ...
    // 2. Handle Errors (e.g., Email not found in DB)
    if (error) {
      toast.error(error);
      dispatch(resetAuthSlice()); 
    }

    // 3. Handle Success Message (e.g., "Password reset link sent!")
    if (message) {
      toast.success(message);
      dispatch(resetAuthSlice());
      
      // Optionally, you could navigate them back to login immediately:
      // navigate("/login");
    }
    ...
    <div className="flex flex-col md:flex-row h-screen bg-white">
      ...
      <div className="hidden md:flex w-full md:w-1/2 bg-black text-white flex-col justify-center items-center p-8 rounded-tr-[80px] rounded-br-[80px] z-10 shadow-2xl">
        <div className="text-center mb-12 flex flex-col items-center">
          <KeyRound size={80} className="mb-6 text-brand-primary drop-shadow-[0_0_15px_rgba(33,212,30,0.8)]" />
          <h1 className="text-5xl font-bold tracking-wider mb-4">Account Recovery</h1>
          <p className="text-gray-400 text-lg max-w-sm leading-relaxed">
            Don't worry, it happens to the best of us. Let's get you back into your digital library.
          </p>
        </div>

        <div className="text-center">
          <p className="text-gray-300 mb-4">Remembered your password?</p>
          <Link
            to="/login"
